...
void Renderer::render(
	...
	for (auto mesh : meshes) {
		...
		for (auto face : mesh.faces) {
			...
			faceWorld.adjacentFaces = faceMesh.adjacentFaces;
	...
	for (auto information : toRender) {
		...
		auto edges = map<FaceRenderInformation::OutputEdge, Edge2d>({
			{ FaceRenderInformation::OutputEdge::P01, { Vertex2d(information.projected.p[0]), Vertex2d(information.projected.p[1]) } },
			{ FaceRenderInformation::OutputEdge::P12, { Vertex2d(information.projected.p[1]), Vertex2d(information.projected.p[2]) } },
			{ FaceRenderInformation::OutputEdge::P20, { Vertex2d(information.projected.p[2]), Vertex2d(information.projected.p[0]) } }
		});

		for (auto const& [outputEdge, edge] : edges) {
			if (information.outputEdge & outputEdge) {
				renderResult.lines.push_back({
					edge,
					edgeColor
				});
			}
